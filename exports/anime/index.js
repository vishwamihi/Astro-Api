...
const getFolderNames = (rootDir) => {
  return readdirSync(rootDir).filter((file) => statSync(join(rootDir, file)).isDirectory())
}
...
const getFilesFromFolders = (rootDir) => {
  ...
  folders.forEach((folder) => {
    ...
    const files = readdirSync(folderPath).filter((file) => statSync(join(folderPath, file)).isFile())
